...
public class StringLOngestWord {
    ...
    public static void main(String[] args) {
        ...
        //  Find third largest length string
        ...
        //  1st way: Using max() with Comparator.comparingDouble
        Optional<employee> max = list.stream()
            .max(Comparator.comparingDouble(e -> e.salary));
        System.out.println("Highest salary employee (comparingDouble): " + max);

        //  2nd way: Using reduce() to compare salaries
        Optional<employee> reduce = list.stream()
            .reduce((s1, s2) -> s1.salary > s2.salary ? s1 : s2);
        System.out.println("Highest salary employee (reduce): " + reduce);

        //  3rd way: Using max() with Double.compare
        Optional<employee> max2 = list.stream()
            .max((s1, s2) -> Double.compare(s1.salary, s2.salary));
        System.out.println("Highest salary employee (Double.compare): " + max2);
    }
}
